...
class Batch(DotDict):
    ...
    def __init__(
        ...
        cold_starts: Optional[torch.Tensor] = None, 
        **kwargs
    ):
    ...
    def _validate_args(self):
        ...
        if self.cold_starts is not None and self.cold_starts.shape != (self.size, self.max_seq_len):
            raise ValueError(
                f"cold_starts must be of shape (batch_size={self.size},"
                f" max_seq_len={self.max_seq_len}), got {self.cold_starts.shape}"
            )
    ...
    def from_list(sequences: List[Sequence]):
        ...
        if sequences[0].cold_starts is not None:
            cold_starts = pad_sequence([seq.cold_starts for seq in sequences], max_len=max_seq_len)
        else:
            cold_starts = None

        return Batch(inter_times, mask, marks, cold_starts)
    ...
    def get_sequence(self, idx: int) -> Sequence:
        ...
        if self.cold_starts is not None:
            cold_starts = self.cold_starts[idx, :length - 1]
        else:
            cold_starts = None
        # TODO: recover additional attributes (passed through kwargs) from the batch
        return Sequence(inter_times=inter_times, marks=marks, cold_starts=cold_starts)
