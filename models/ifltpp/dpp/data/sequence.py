...
class Sequence(DotDict):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cold_starts is not None:
            if not isinstance(cold_starts, torch.Tensor):
                cold_starts = torch.tensor(cold_starts)
            self.cold_starts = cold_starts.long()
        else:
    ...
    def _validate_args(self):
        ...
        if self.cold_starts is not None:
            expected_cold_starts_length = len(self.inter_times) - 1
            if self.cold_starts.shape != (expected_cold_starts_length,):
                raise ValueError(
                    f"wait times must be of shape (seq_len - 1 = {expected_cold_starts_length},)"
                    f"(got {self.cold_starts.shape})"
                )
