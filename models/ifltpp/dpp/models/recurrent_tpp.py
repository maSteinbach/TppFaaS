...
class RecurrentTPP(nn.Module):
    ...
    def __init__(
        ...
        rnn_type: str = "GRU",
        coldstart_feature: bool = False,
        coldstart_embedding_size: int = 32
    ):
        super().__init__()
        ...
        if self.num_marks > 1:
            ...
            self.num_features = 1
        self.coldstart_feature = coldstart_feature
        if self.coldstart_feature:
            self.num_features += coldstart_embedding_size
            self.coldstart_embedding = nn.Embedding(2, coldstart_embedding_size)
        self.rnn_type = rnn_type
        self.context_init = nn.Parameter(torch.zeros(context_size))  # initial state of the RNN
        self.time_linear = nn.Linear(self.context_size, 1)
    ...
    def get_features(self, batch: dpp.data.Batch) -> torch.Tensor:
        ...
        if self.coldstart_feature:
            coldstart_emb = self.coldstart_embedding(batch.cold_starts)
            features = torch.cat([features, coldstart_emb], dim=-1)
    ...
    def mark_log_prob(self, batch: dpp.data.Batch) -> torch.Tensor:
        """Compute the mark log-likelihood for a batch of sequences.
    ...
    def predict(self, batch: dpp.data.Batch) -> (torch.Tensor, Optional[torch.Tensor]):
        """
        Args:
            batch: Batch of sequences in padded format (see dpp.data.batch).

        Returns:
            inter_times: Predicted inter_times,
        ...
        features = self.get_features(batch)
        context = self.get_context(features)
        #inter_times = self.get_inter_time_dist(context).mean                   # (batch_size, seq_len)
        inter_times = self.softplus(self.time_linear(context)).squeeze()        # (batch_size, seq_len)
        if self.num_marks > 1:
            mark_logits = torch.log_softmax(self.mark_linear(context), dim=-1)  # (batch_size, seq_len, num_marks)
            marks = mark_logits.argmax(dim=-1)  # (batch_size, seq_len)
        else:
            marks = None
        return inter_times, marks
